...
import (
	...
	"github.com/QuantumNous/new-api/relay/channel/astraflow"
...
func GetAdaptor(apiType int) channel.Adaptor {
	...
	case constant.APITypeAstraFlow:
		return &astraflow.Adaptor{}
...
// forkTaskAdaptor returns the fork's own Go task adaptor for platforms whose
// per-channel logic is kept downstream (doubao / astraflow — including the
// Ark-style video endpoints and ArkVideoConverter). Migrated platforms are
// served by JS plugins instead.
func forkTaskAdaptor(platform constant.TaskPlatform) channel.TaskAdaptor {
...
func GetTaskAdaptor(platform constant.TaskPlatform) channel.TaskAdaptor {
	if adaptor := forkTaskAdaptor(platform); adaptor != nil {
		return adaptor
	}
...
// from one registry generation before the adaptor is returned. Platforms that
// keep a fork Go adaptor (doubao / astraflow) resolve to it directly.
