...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	defer func() {
		if newAPIError != nil {
			...
			// 流错误出口(returnOpenAIStreamError)已幂等补过 request id 时跳过,
			// 避免 message 尾部重复追加 "(request id: …)"。
			if !strings.Contains(newAPIError.Error(), "(request id: ") {
	...
	// 把估算构成分解挂到预扣明细上，随消费日志输出（请求作用域）
	if bd := service.GetTokenEstimateBreakdown(c); bd != nil && priceData.PreConsumeDetail != nil {
