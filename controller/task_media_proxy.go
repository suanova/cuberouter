// 本文件承载上游（QuantumNous/new-api）的 task-media 内容代理实现。
//
// 上游把该逻辑放在 controller/video_proxy.go 中；fork 的 controller/video_proxy.go
// 是本地保留的 Gemini/Vertex 视频内容代理（video_proxy_gemini.go），在同步冲突中
// 以 fork 版本胜出，导致上游 controller/task.go 引用的 proxyTaskMedia 与
// writeTaskMediaProxyError 丢失。这里把被覆盖的上游函数原样恢复，
// 使任务工件（task artifact）的内容代理链路保持完整。
package controller
