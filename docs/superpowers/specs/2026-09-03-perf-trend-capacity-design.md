...
1. **趋势图增强**：模型级趋势线支持 P50/P95/P99 开关（默认 P95/P99 开），图表库复用现有 `LatencyTrendChart` 所用库（plan 阶段确认并遵守 web/AGENTS 图表可访问性约定）；
2. **渠道维度**：模型详情/对比视图新增「按渠道分」——同模型下各渠道成功率、延迟、TPS 对比与逐渠道趋势；
